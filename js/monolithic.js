...
var soejs = {
	...
	// for 2020, see also chartsLoaded3
	...
	// for 2020, 
	// some pages use server side js to render all tables
	// so a lot of stuff that was previously done not needed
	// as is, works only with those that don't have region tabs
	...
	chartsLoaded3: function () {
		...
		// hide everything not region-queensland
		...
		document.querySelectorAll("ul.chart-tabs").forEach(function (ul) {
			...
			ul.addEventListener("click", function (evt) {
				// the click event is on the span, but the parent <li> has the class
				if (!evt.target.parentNode.classList.contains("active")) {// don't do anything if they click the already active tab
